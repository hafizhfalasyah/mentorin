...
class SoalController extends Controller
{
    public function viewSoal($id_paket){
        ...
        if($paket==null) return redirect(url('/try-out'))->withErrors(['error'=>'Paket tidak tersedia']);
        ...
        else if($riwayat_paket->status==1 and $riwayat_paket->paketan==null and $paket->latihan_soal==0){
    ...
    public function viewAturan($id_paket){
        ...
        else if($riwayat_paket==null or $paket->latihan_soal==1) return view('user.soal/soal-aturan')->with([
            'user'=>$user, 
            'paket'=>$paket, 
            'noted_time'=>$noted_time
        ]);
    ...
    public function viewHasil($id_paket){
        ...
        else if(substr($id_paket, 3, 3)=="DKD"){
            $riwayat_pakets = RiwayatPaket::where('id_user', Auth::id())->where('paketan', $id_paket)->get();
            foreach($riwayat_pakets as $r){
                $paket_ini = Paket::where('id', $r->id_paket)->first();
                $nama_paket = $paket_ini->nama_paket;
    ...
    public function submitJawaban(Request $request){
        ...
        for($i = 1; $i <= count($soals); $i++){
            if($jawabans->has('answer'.$i)){
                ...
                if(strlen($j)==1) $pointed_jawaban = $pointed_jawaban->where('nomor_soal', $i)->where('nomor_jawaban', (int)$j)->get()->first();
                else $pointed_jawaban = $pointed_jawaban->where('nomor_soal', $i)->where('nomor_jawaban', 1)->get()->first();
    ...
    public function viewSolusi($id_paket){
        ...
        if($paket->latihan_soal==1 and $order->status!=2) return redirect(url('/exercise'))->withErrors(['error'=>'Anda belum mengerjakan Latihan Soal ini']);
        else if($riwayat_paket==null or $riwayat_paket->status==0) return redirect(url('/try-out'))->withErrors(['error'=>'Anda belum mengerjakan Paket ini']);
        ...
        foreach($soals as $soal){
            ...
            foreach($sub_jawabans as $sj){
                ...
                if(strstr($riwayat_jawaban[$count], $sj->nomor_jawaban) and strstr($soal->nomor_jawaban, $sj->nomor_jawaban)==false) $sj->benar_jawaban = 2;
                if(strstr($riwayat_jawaban[$count], $sj->nomor_jawaban) and strstr($soal->nomor_jawaban, $sj->nomor_jawaban)) $sj->benar_jawaban = 3;
                }
                $jawabans->add($sj);
            }
            $count++;
    ...
    public function viewAturanPaket($id_paket, $type){
        ...
        else if($riwayat_paket->status==1){
            return redirect(url('history'))->withErrors(['error'=>'Anda sudah mengerjakan paket ini!']);
        } 
        else{
            $noted_time = $riwayat_paket->time;
        ...
        if($riwayat_paket==null) return view('user.soal/soal-aturan')->with([
            'user'=>$user, 
            'paket'=>$paket,
            'paketan'=>$id_paket, 
            'noted_time'=>$noted_time,
    ...
    public function submitJawabanPaket(Request $request){
        ...
        else if(substr($hasil['id_paket'], 3, 3)=="DKD"){
            $id_type = collect([
        ...
        for($i = 1; $i <= count($soals); $i++){
            if($jawabans->has('answer'.$i)){
                ...
                else if($j==$soals->where('nomor_soal', $i)->first()->nomor_jawaban or $pointed_jawaban->benar_jawaban==1){
                    $benar++;
                    $score += $pointed_jawaban->skor_jawaban;
                }
                else{
        ...
        if(substr($hasil['id_paket'], 3, 3)=="PSI"){
            foreach($riwayat_paket_type as $key => $r){
                if(substr($r->id_paket, 3, 3)=="PPH") continue;
                if(substr($r->id_paket, 3, 3)=="PKC") $score_total += $r->score*20/100;
                if(substr($r->id_paket, 3, 3)=="PKR") $score_total += $r->score*55/100;
                if(substr($r->id_paket, 3, 3)=="PCM") $score_total += $r->score*25/100; 
            }
        }
        else if(substr($hasil['id_paket'], 3, 3)=="DKD"){
            foreach($riwayat_paket_type as $key => $r){
    ...
    public function viewSolusiPaket($id_paket, $type){
        ...
        $jawabans = collect();
        $riwayat_paket = RiwayatPaket::where('id_user', Auth::id())->where('id_paket', $id_paket)->get()->first();
        if($riwayat_paket==null or $riwayat_paket->status==0) return redirect(url('/try-out'))->withErrors(['error'=>'Anda belum mengerjakan Paket ini']);
        $riwayat_jawaban = $riwayat_paket->riwayat_jawaban;
        $riwayat_jawaban = explode("_", $riwayat_jawaban);
        $count = 0;
        foreach($soals as $soal){
